...
import { X, User, MapPin, Utensils, DollarSign, Heart, TrendingUp } from 'lucide-react';
...
export function UserDetailPanel({ userId, currentUserId, onClose }: UserDetailPanelProps) {
          ...
          {loading || !user ? (
            ...
            <div className="h-full overflow-y-auto p-6 space-y-6">
              ...
              <div className="flex flex-col items-center text-center space-y-3 pt-8">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-800">{user.display_name}</h2>
              ...
              {user.preferences && (
                <>
                  {/* Favorite Cuisines */}
                  {user.preferences.favorite_cuisines && user.preferences.favorite_cuisines.length > 0 && (
                  ...
                  {user.preferences.favorite_restaurants && user.preferences.favorite_restaurants.length > 0 && (
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-slate-700">
                      ...
                      <div className="space-y-1">
                        {user.preferences.favorite_restaurants.slice(0, 5).map((restaurant) => (
                          <div
                            key={restaurant}
                  ...
                  {/* Taste Profile */}
                  {user.preferences.taste_profile && Object.keys(user.preferences.taste_profile).length > 0 && (
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {Object.entries(user.preferences.taste_profile).map(([taste, score]) => (
                          <div key={taste}>
                            <div className="flex justify-between text-xs mb-1">
                              <span className="text-slate-600 capitalize">{taste}</span>
                              <span className="text-slate-800 font-semibold">{(score * 100).toFixed(0)}%</span>
                            </div>
                  ...
                  {user.preferences.price_preference && (
                    <div className="space-y-2">
                      ...
                      <div className="px-3 py-2 rounded-lg glass-btn inline-block">
                        <span className="text-sm font-medium">{user.preferences.price_preference}</span>
                      </div>
                    </div>
                  )}
